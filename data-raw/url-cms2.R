...
S7::method(req_total, base_cms) <- function(x) {
  ...
  if (x@multi) {
    N <- multi_count(url, x@url, "total_rows")
    cli_total2(N, x@end)
  } else {
    N <- base_request(url, "total_rows")
    cli_total(N, x@end)
  }
  invisible(N)
}
...
#' @noRd
S7::method(req_count, base_cms) <- function(x, args) {
  url <- flatten_url(paste0(x@url, "/stats?"), build(args))

  if (x@multi) {
    N <- multi_count(url, x@url, "found_rows")
  } else {
    N <- base_request(url, "found_rows")
  }
  invisible(N)
}

#' @noRd
S7::method(req_single, base_cms) <- function(x, args, id = NULL) {
  url <- flatten_url(paste0(x@url, "?"), build(args), opts_cms())
