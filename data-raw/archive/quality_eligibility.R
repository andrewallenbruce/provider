...
quality_eligibility <- function(
  ...
  url <- glue::glue(
    "https://qpp.cms.gov/api/eligibility/npis/{npi}/?year={year}"
  )
  ...
  if (tidy) {
    results <- results |>
      ...
      dplyr::rename(
        ind = organizations_individualScenario,
    ...
    if (unnest) {
      ...
      results <- tidyup(
        ...
        dplyr::mutate(
          npi_type = fct_entype(npi_type),
          org_state = fct_stabb(org_state)
        ) |>
        make_interval(
          start = first_approved_date,
          end = lubridate::ymd(paste0(year, '-12-31'))
        ) |>
        dplyr::mutate(
          years_in_medicare = round(timelength_days / 365),
          .after = first_approved_date
        ) |>
        dplyr::mutate(interval = NULL, period = NULL, timelength_days = NULL) |>
        dplyr::group_by(year) |>
...
unnest_if_name <- function(df, name, unpack = TRUE, wide = FALSE) {
  if (rlang::has_name(df, name)) {
    if (wide) {
      return(tidyr::unnest_wider(df, name, names_sep = "."))
    }
...
cols_qelig <- function(df, type = c('tidyup', 'top', 'apms', 'ind', 'grp')) {
  ...
  if (type == 'apms') {
    ...
    results <- df |>
      dplyr::select(year, npi, org_name, dplyr::contains('apms_')) |>
  ...
  if (type == 'ind') {
    ...
    results <- df |>
      dplyr::select(year, npi, org_name, dplyr::contains('ind_')) |>
  ...
  if (type == 'grp') {
    ...
    results <- df |>
      dplyr::select(year, npi, org_name, dplyr::contains('grp_')) |>
