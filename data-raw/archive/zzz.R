...
#' @export
distros_dac <- \() {
  ids <- c(affiliations = "27ea-46a8", clinicians = "mj5m-pzi6")
  ...
  affiliations <- list(
    dataset = c(title = af$title, description = af$description),
    ids = c(
    ...
    urls = c(landing = af$landingPage, csv = af$distribution$data$downloadURL)
  )

  clinicians <- list(
    dataset = c(title = cl$title, description = cl$description),
    ids = c(
    ...
    urls = c(landing = cl$landingPage, csv = cl$distribution$data$downloadURL)
  )

  c(affiliations = affiliations, clinicians = clinicians)
}
