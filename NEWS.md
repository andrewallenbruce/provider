...
# provider 0.0.1.9282 (2026-06-30)

* nppes(taxonomy, location) refinement
...
* `cli_progress_step` instances now use `withr::defer_parent` on exit
...
* `set_rename`/`get_columns` now uses class of object to retrieve name remapping scheme
...
* `quality()` polish method refactoring
...
* `quality()` gets another pass


# provider 0.0.1.9200 (2026-06-01)

* `quality()` polish and pivot methods
...
# provider 0.0.1.9177 (2026-05-17)
...
# provider 0.0.1.9162 (2026-05-12)
...
* NOTE: odd rcmdcheck warnings, re: the `c()` function?:
...
* `hrsa` functionality,. `nppes()` refactoring
