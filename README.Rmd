...
<br>
 ...
 -  [Medicare Market Saturation & Utilization State-County](https://data.cms.gov/summary-statistics-on-use-and-payments/program-integrity-market-saturation-by-type-of-service/market-saturation-utilization-state-county)
...
```{r echo=FALSE}
mooa_ex |> 
  dplyr::mutate(days_to_end = age_days(start_date = optout_end_date, end_date = lubridate::today()))
  dplyr::select(
    NPI = npi,
    Last = last_name,
    First = first_name,
    ...
    State = state_code) |>
  dplyr::mutate(
    optout_effective_date = datefixR::fix_date_char(
    optout_effective_date, format = "mdy"),
    optout_end_date = datefixR::fix_date_char(optout_end_date, format = "mdy")) |> 
  dplyr::rename("Opt-Out Begins" = optout_effective_date, "Opt-Out Ends" = optout_end_date) |> 
  knitr::kable()
```
