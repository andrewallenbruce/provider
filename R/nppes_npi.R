...
#' @param entype Choices are either `I` for an Individual provider (NPI-1 or
#'    Type 1) or `O` for an Organizational provider (NPI-2 or Type 2.). When
#'    not specified, both Type 1 and Type 2 NPIs will be returned. Entype
#'    cannot be the only criteria entered.
#' @param first_name Individual provider's (NPI-1) first name. Trailing wildcard
...
#' @autoglobal
#' @noRd
nppes_summary <- function(npi            = NULL,
                      entype         = NULL,
                      first_name     = NULL,
                      last_name      = NULL,
                      org_name       = NULL,
