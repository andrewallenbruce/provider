...
utilization <- function(
  ...
  if (type != 'Provider') {
    c(nest, detailed) %<-% c(FALSE, FALSE)
  }
  if (type == 'Provider') {
    c(rbcs, hcpcs = NULL, pos = NULL, drug = NULL) %<-% c(FALSE)
  } # nolint
  if (type != 'Geography') {
...
tidyup_geography.util <- function(results, rbcs) {
  results <- tidyup(
    ...
    dplyr::mutate(
      rndrng_prvdr_geo_desc = fct_stname(rndrng_prvdr_geo_desc),
