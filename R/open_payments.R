...
open_payments <- function(
  ...
  if (!is.null(covered_type)) {
    covered_type <- rlang::arg_match(
      covered_type,
      c("Physician", "Non-Physician Practitioner", "Teaching Hospital")
    )
  ...
  url <- paste0(
    "https://openpaymentsdata.cms.gov/api/1/datastore/sql?query=",
    "[SELECT * FROM ",
    id,
    "]",
  ...
  if (tidy) {
    ...
    results <- tidyup(
      ...
      yn = c(yncols), #nolint
      dbl = 'dollars',
      int = c('program_year', 'number_of_payments_included_in_total_amount'),
      zip = 'recipient_zip_code'
    ) |>
      dplyr::mutate(
        covered_recipient_type = fct_cov(covered_recipient_type),
        recipient_state = fct_stabb(recipient_state),
        applicable_manufacturer_or_applicable_gpo_making_payment_state = fct_stabb(
          applicable_manufacturer_or_applicable_gpo_making_payment_state
        ),
        covered_recipient_license_state_code1 = fct_stabb(
  ...
  if (pivot) {
    results <- results |>
      ...
      dplyr::rowwise() |>
      dplyr::mutate(
        total = sum(dplyr::c_across(.name_1:.pdi_1), na.rm = TRUE),
    ...
    pcol <- c('name_', 'covered_', 'type_', 'category_', 'ndc_', 'pdi_') %s+%
      rep(1:5, each = 6)
...
open_ids <- function(search) {
  ...
  results <- dplyr::tibble(
    httr2::resp_body_json(response, check_type = FALSE, simplifyVector = TRUE)
  ) |>
    ...
    dplyr::mutate(
      year = strex::str_before_first(title, " "),
      set = strex::str_after_first(title, " "),
      .before = 1,
...
nature <- function(x) {
  dplyr::case_match(
    ...
    paste(
      ...
      "education program"
    ) ~ "Compensation Other Than Consulting",
    .default = x
  )
}
