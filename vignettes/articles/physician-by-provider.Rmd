...
stats <- quality_payment_years() |> 
  map(\(x) quality_stats(year = x)) |> 
  list_rbind()

all |> select(-c(demographics, conditions))
```
...
all |> 
  ...
  select(year, dual_eligibility_ratio)
```
...
stats |> 
  filter(type == "Individual", 
         stat == "Dual Eligibility Average") |> 
  select(year, 
         dual_eligibility_ratio = value)
```
...
```{r}
service <- by_service_years() |> 
  map(\(x) by_service(year = x, npi = 1023076643)) |> 
  list_rbind() |> 
  mutate(level = "individual")

geography <- by_service_years() |> 
  map(\(x) by_service(year = x, npi = 1023076643)) |> 
  list_rbind() |> 
  select(year, state, hcpcs_code, pos) |> 
  rowwise() |> 
  mutate(statewide = by_geography(year, state, hcpcs_code, pos),
         national = by_geography(year, state = "National", hcpcs_code, pos))
```
...
bind_rows(
  ...
  pivot_wider(names_from = level, 
              values_from = beneficiaries:avg_payment) |> 
  pivot_longer(!year:pos, 
               names_to = "stat", 
               values_to = "amount") |> 
  separate_wider_regex(stat, c(measure = ".*", "_", level = ".*")) |> 
  pivot_wider(names_from = level, 
              values_from = amount) |> 
  filter(measure %in% c("beneficiaries", "services")) |> 
  arrange(measure, hcpcs_code) |> 
  gt::gt() |> 
