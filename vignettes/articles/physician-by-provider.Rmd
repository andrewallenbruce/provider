...
```{r}
srvc <- by_service_years() |> 
  map(\(x) by_service(year = x, npi = 1023076643)) |> 
  list_rbind() |> 
  select(year,  hcpcs = hcpcs_code, pos)
srvc
# year <- as.list(srvc$year)
# hcpcs_code <- as.list(srvc$hcpcs)
# pos <- as.list(srvc$pos)
# 
# l <- list(year = srvc$year, hcpcs_code = srvc$hcpcs, pos = srvc$pos)
# 
# pmap(l, by_geography(year, hcpcs_code, pos, level = "National"))
```


```{r}
by_geography(year = 2013, hcpcs_code = "99217", pos = "F")
```
