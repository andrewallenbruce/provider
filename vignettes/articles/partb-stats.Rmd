...
```{r}
lm.performance <- lm(tot_payment ~ tot_benes + .srvcs_per_bene + .pymt_per_srvc + .copay_deduct + .pymt_per_bene, data = performance)
lm.performance
...
```{r}
# ggplot(performance, aes(x = .pymt_per_srvc, y = .srvcs_per_bene, fill = year, group = year)) +
