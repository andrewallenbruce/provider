...
val |> 
  ...
  summarise(
    n = n(),
    avg_hcpcs = mean(tot_hcpcs),
    avg_benes = mean(tot_benes),
    avg_srvcs = mean(tot_srvcs),
    avg_charges = mean(tot_charges),
    avg_allowed = mean(tot_allowed),
    avg_payment = mean(tot_payment),
    avg_hcc = mean(hcc_risk_avg), .groups = "drop") |> 
  arrange(entity_type) |>
  group_by(entity_type) |>
  change(c(n, starts_with("avg_"))) |> 
  ungroup() |>
  select(-ends_with("_cum")) |> 
  select(year, entity_type, ends_with("_chg")) |> 
  filter(year != 2013) |>
  gt(rowname_col = "year", groupname_col = "entity_type") |> 
...
```{r}
select(ind, npi:specialty, city:state) |>
  distinct() |> 
...
```{r}
select(ind, year, tot_hcpcs:tot_payment) |>
  gt(rowname_col = "year") |> 
  opt_table_font(font = google_font(name = "JetBrains Mono")) |> 
