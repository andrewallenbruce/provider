...
The Opt Out Affidavits dataset provides information on providers who have decided not to participate in Medicare. It contains the provider's NPI, specialty, address, and effective dates.
...
Contact your Medicare Administrative Contractor (MAC) to see what information you should include in your opt-out affidavit and private contract.
...
(out <- opt_out(last = "Smith"))
```

<br>

Filter to those providers that are eligible to order and refer and use the `order_refer()` function to retrieve more information on their particular eligibilities:

<br>

```{r message=FALSE, warning=FALSE}
(ord <- opt_out(last = "Smith") |> 
   dplyr::filter(order_and_refer == TRUE) |> 
  dplyr::distinct(npi) |> 
  dplyr::pull(npi) |>
  furrr::future_map(\(x) order_refer(npi = x)) |> 
  purrr::list_rbind() |> 
...
Then join the two results together for further analysis:

<br>


```{r message=FALSE, warning=FALSE}
dplyr::left_join(out, ord, 
                 by = dplyr::join_by(npi)) |> 
  dplyr::arrange(npi, order_and_refer) |> 
  dplyr::select(NPI = npi, 
                First = first_name, 
                Last = last_name, 
                Specialty = specialty, 
                Eligible = order_and_refer, 
                "Part B" = partb, 
                HHA = hha, 
                DME = dme, 
                PMD = pmd) |> 
