...
row("Find Health Care Providers by Name, Location, NPI number, or Specialty")
```
...
```{r}
x$other_names[[1]] # ONLY NPI-2
```



```{r}
basic <- x$basic[[1]] |> tibble::tibble()
addresses <- x$addresses[[1]] |> tibble::tibble()
 ...
 * `other_names`: Other names used by type 2 (organization) providers only
