---
title: "Medicare Fee-For-Service Public Provider Enrollment API"
...
constructive::construct(mppe_1)
```

```{r echo=FALSE}
mppe_1 <- tibble::tibble(
  Variable = c(
...
flextable::flextable(mppe_1) |> flextable::autofit() |> ftExtra::colformat_md() |> flextable::theme_vanilla()
```
...
mppe_2 <- mppe_2 |> 
  ...
  dplyr::mutate(code = formulaic::add.backtick(code, include.backtick = "all")) |> 
  tidyr::separate(description, into = c("type", "subtype"), sep = " - ", remove = TRUE, extra = "merge") |> 
  dplyr::select(Code = code, Type = type, Subtype = subtype)

constructive::construct(mppe_2)
```

```{r echo=FALSE}
mppe_2 <- tibble::tibble(
  Code = c(
...
flextable::flextable(mppe_2) |> flextable::autofit() |> ftExtra::colformat_md() |> flextable::theme_vanilla()
```
