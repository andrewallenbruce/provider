...
dplyr::left_join(yr_nat_base, 
  ...
  gt::fmt_number(columns = c(bene_total), decimals = 0) |> 
  ...
  gt::sub_missing(columns = dplyr::everything(), 
                  missing_text = "--") |> 
  # gtExtras::gt_merge_stack(col1 = bene_total_chg, 
  #                          col2 = bene_total_pct) |> 
...
mon_nat_base <- national |> 
  ...
  dplyr::mutate(cummean = dplyr::cummean(bene_total)) |>
  provider:::change_abs(bene_total, year) |> 
  provider:::change_pct(bene_total, bene_total_chg, year) |> 
  dplyr::relocate(cummean, .after = bene_total)
