...
```{r}
p <- providers(pac = 7810891009) |> mutate(source = '`providers()`', .before = 1)
r <- reassignments(pac = 7810891009) |> mutate(source = '`reassignments()`')
c <- clinicians(pac = 7810891009) |> mutate(source = '`clinicians()`')
...
```{r}
a <- affiliations(pac = 7810891009) |> select(-facility_type)

h <- a |> pull(facility_ccn) |>
