...
hospitals(organization = "Elizabethtown Community Hospital") |> 
  select(npi, organization, pac_id_org, facility_ccn, specialty_code)
```
...
affiliations(facility_ccn = 331302) |> 
  select(-c(middle_name, suffix, parent_ccn)) |> 
  head(10)
```
...
```{r}
laboratories(city = "Valdosta", certificate = "waiver") |> 
  dplyr::select(name, clia, certificate, expiration_date, termination_reason, facility_type) |> 
  janitor::remove_empty(which = c("rows", "cols")) |> 
