...
The goal of `provider` is to make the experience of accessing publicly-available Medicare provider data easier and more consistent across a variety of sources.
...
hosp_2 <- hos |> 
  ...
  gt::cols_label(org_name = "Organization Name", 
                 ...
                 incorp_length = "Years Incorporated",
...
re_assign <- revalidation_reassign(npi = 1043477615) |> 
  dplyr::select(group_legal_business_name, 
                group_state, 
                group_pac_id, 
                group_enroll_id, 
                group_reassignments) |> 
  dplyr::mutate(group_state = campfin::expand_state(group_state)) |> 
  ...
  gt::cols_label(group_legal_business_name = "Business Name", 
                 group_state = "State", 
                 group_reassignments = "Reassignments",
                 group_pac_id = "PAC ID", 
                 group_enroll_id = "Enrollment ID")
```
...
provider_1 <- provider |> 
  ...
  gt::data_color(columns = c("tot_charges", "tot_allowed", "tot_payment"), palette = "ggsci::red_material")
```
...
qpp |> 
  dplyr::select(year, 
                ...
                Complex = complex_patient_bonus,
                Quality = quality_category_score,
                QI = quality_improvement_bonus,
                PI = pi_category_score,
                IA = ia_score,
                Cost = cost_score) |> 
  provider:::gt_prov()
```
...
qpp |> 
  dplyr::select(year, 
                ...
                "Rural Clinician" = rural,
                "HPSA Clinician" = hpsa,
                "Ambulatory Surgical Center" = asc,
                "Hospital-Based" = hospital_based,
                "Non-Patient Facing" = non_patient_facing,
                "Facility-Based" = facility_based,
                "Performance Extreme Hardship" = extreme_hardship,
                "Quality Performance Bonus" = quality_bonus,
                "Quality Performance Extreme Hardship" = extreme_hardship_quality, 
                "Promoting Interoperability Hardship" = pi_hardship,
                "Promoting Interoperability Extreme Hardship" = extreme_hardship_pi,
                "Promoting Interoperability Reweighting" = pi_reweighting,
                "Promoting Interoperability Bonus" = pi_bonus, 
                "Improvement Activities Extreme Hardship" = extreme_hardship_ia, 
                "Improvement Activities Study" = ia_study, 
                "Cost Performance Extreme Hardship" = extreme_hardship_cost) |> 
  tidyr::pivot_longer(cols = !year, 
                      names_to = "status", 
                      values_to = "value") |> 
  tidyr::pivot_wider(names_from = year, 
                     values_from = value) |> 
  provider:::gt_prov(clean = FALSE, 
                     divider = c("status"),
                     checkmark = c("2017", 
                                   "2018", 
                                   "2019", 
...
```{r}
quality_eligibility(year = 2021, npi = 1043477615)
```
