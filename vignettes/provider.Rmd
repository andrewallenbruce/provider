...
The following return yearly Provider-specific stats:

   * `by_provider()`: Utilization summary.
   * `by_service()`: Utilization detail.
   * `by_geography()`: National/state/county utilization summary.
   * `quality_payment()`: QPP/MIPS performance.
   * `quality_eligibility()`: QPP/MIPS eligibility.
   * `open_payments()`: Open Payments reporting.
   * `beneficiaries()`: Medicare enrollment.
   * `chronic_conditions`: Chronic condition prevalence.
...
```{r}
map_dfr(quality_years(), ~quality_payment(year = .x, npi = 1043477615)) |> 
...
pending(type = "physician", npi = 001234569123)
```
...
revalidation_date(enid = "I123456789123456")

# Must begin with a capital I (Individual) or O (Organization/Group)
providers(enid = "012345678912345")
