...
```{r}
c(1285636522, 1730198755, 1427145176) |> 
  purrr::map(nppes_npi) |> 
  purrr::list_rbind() 
```
...
order_refer(npi = 1083879860) |> 
  gt::gt() |> 
  gt::tab_header(title = "Medicare Order and Referring API") |> 
  gt::text_transform(
    locations = gt::cells_body(columns = c(partb, dme, hha, pmd)),
    fn = function(x) {if (x == as.character("TRUE")) 
      {gt::html(fontawesome::fa("circle-check", 
                                prefer_type = "solid", 
                                fill = "green", height = "1.5em", width = "1.5em"))
      } else if (x == as.character("FALSE")) 
      {gt::html(fontawesome::fa("circle-xmark", 
                                  prefer_type = "solid", 
                                fill = "red"))}})
```
