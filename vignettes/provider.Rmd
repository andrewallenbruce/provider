...
### Administrative
   ...
   * `provider()`: Providers actively enrolled in Medicare.
   * `opt_out()`: Providers currently opted out of Medicare.
   * `revalidation()`: Link Type-1 and Type-2 providers.
   * `clinician()`: Provider school, graduation year, linking.
...
The following return provider-specific stats:
   ...
   * `by_physician()`: Yearly high-level utilization data.
   * `by_service()`: Yearly detailed utilization data.
   * `qpp_performance()`: Yearly QPP/MIPS performance data.
   * `qpp_eligibility()`: Yearly QPP/MIPS eligibility data.
...
### Auxiliary
   ...
   * `endpoints_missing()`: Is a provider missing Endpoints in the NPPES NPI Registry?
   ...
   * `qpp_stats()`: Yearly QPP/MIPS performance data.
...
```{r}
# Must be 10 characters long
affiliations(pac_id = 0123456789)
...
clinicians(enroll_id = 0123456789123456)

# Must be 15 characters long
revalidation(enroll_id = "I123456789123456")

# Must begin with a capital I (Individual) or O (Organization/Group)
providers(enroll_id = "012345678912345")

# Some functions' parameters differentiate between an Organizational and Individual ID
